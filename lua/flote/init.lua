...
local open_float = function(file, style, border)
...
M.setup = function(opts)
	...
	local border = opts.border or defaults.border
	local style = opts.style or defaults.style
	...
	vim.api.nvim_create_user_command("Flote", function(args)
		...
		elseif args.fargs[1] == "manage" then
			...
			open_float(note_file, style, border)
		end
	end, {
		nargs = "?",
